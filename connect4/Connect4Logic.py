...
class Board():
    ...
    def add_stone(self, column, player):
        ...
        #print(np.where(self.np_pieces[:, column] == 0))
    ...
    def _is_diagonal_winner(self, player_pieces):
        ...
        for i in range(len(player_pieces) - win_length + 1):
            for j in range(len(player_pieces[0]) - win_length + 1): #going right direction
                if all(player_pieces[i + x][j + x] for x in range(win_length)):
                    return True
            for j in range(win_length - 1, len(player_pieces[0])): #going left direction
                if all(player_pieces[i + x][j - x] for x in range(win_length)):
                    return True
        return False
